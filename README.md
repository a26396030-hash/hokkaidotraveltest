...
<head>
    ...
    <style>
        /* 自定義滾動條樣式，讓它在 Tailwind 預設外觀上更美觀 */
        ...
        <div class="bg-white p-6 rounded-xl shadow-2xl relative custom-scrollbar overflow-y-auto max-h-[80vh]">
                ...
                <div class="space-y-4">
                    <div v-for="(item, index) in currentItinerary" :key="item.id"
                        ...
                        <div v-if="item.editing" class="flex flex-col space-y-2">
                            <input type="text" v-model="item.time" placeholder="時間 (例如: 08:00)"
                                   class="p-2 border border-blue-300 rounded focus:ring-blue-500 focus:border-blue-500 text-base font-semibold"/>
                            <textarea v-model="item.activity" placeholder="地點/活動詳情"
                                      class="p-2 border border-blue-300 rounded focus:ring-blue-500 focus:border-blue-500 text-base h-20"></textarea>
                            <div class="flex justify-end space-x-2 mt-2">
                                <button @click="saveItem(item)" class="px-3 py-1 bg-green-500 text-white rounded text-sm hover:bg-green-600 transition">儲存</button>
                                <button @click="cancelEdit(item, index)" class="px-3 py-1 bg-gray-400 text-white rounded text-sm hover:bg-gray-500 transition">取消</button>
                            </div>
                        </div>
            ...
            <div v-else-if="currentTab === 'accounting'" class="min-h-[400px]">
                <h2 class="text-2xl font-bold text-green-700 border-b-2 border-green-200 pb-3 mb-4">
                    記帳系統 <span class="text-lg text-green-500"> (匯率: 1 JPY ≈ {{ exchangeRateTWD.toFixed(4) }} TWD)</span>
                </h2>

                <div class="grid grid-cols-2 gap-4 mb-6 text-center">
                    <div class="bg-blue-50 p-4 rounded-xl shadow-inner border border-blue-200">
                        <p class="text-lg font-medium text-gray-600">總花費 (日幣)</p>
                        <p class="text-3xl font-extrabold text-blue-600 mt-1">{{ totalJPY.toLocaleString() }} ¥</p>
                    </div>
                    <div class="bg-green-50 p-4 rounded-xl shadow-inner border border-green-200">
                        <p class="text-lg font-medium text-gray-600">總花費 (台幣)</p>
                ...
                <div class="bg-gray-100 p-4 rounded-lg mb-6 shadow">
                    <h3 class="font-bold text-gray-700 mb-3">新增交易</h3>
                    <div class="flex space-x-3">
                        <input type="text" v-model="newTransaction.description" placeholder="項目描述 (例如: 晚餐)"
                               class="p-2 border border-gray-300 rounded flex-1 focus:ring-indigo-500" />
                        <input type="number" v-model.number="newTransaction.amountJPY" placeholder="金額 (JPY)"
                    ...
                    <p v-if="newTransaction.amountJPY > 0" class="text-xs text-right text-gray-500 mt-1">
                        約 {{ (newTransaction.amountJPY * exchangeRateTWD).toFixed(0) }} TWD
                    </p>
                </div>
                ...
                <div class="space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
                    <div v-for="tx in sortedAccountingData" :key="tx.id"
                        ...
                        <div class="text-right">
                            <p class="font-bold text-red-500">{{ tx.amountJPY.toLocaleString() }} ¥</p>
                            <p class="text-xs text-gray-500"> ≈ {{ tx.amountTWD.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) }} $</p>
                        </div>
...
<script type="module">
    ...
    const weatherLocations = {
        ...
        'day3': { location: '登別', lat: 42.49, lon: 141.13, desc: '溫泉渡假村/海洋公園' },
        'day4': { location: '函館', lat: 41.77, lon: 140.73, desc: '函館夜景區' },
        'day5': { location: '函館', lat: 41.77, lon: 140.73, desc: '元氣朝市/機場' } 
    };
            ...
            const fetchWeather = async () => {
                for (const day in weatherLocations) {
                    ...
                    try {
                        ...
                        weatherData.value[day] = { location, temp: 'N/A', icon: '❓', desc: '無法取得數據' };
                    }
                }
            };
